...
RNAEnergyEvaluator::RNAEnergyEvaluator(const std::unordered_map<int, std::string> &strands)
    : strands(strands) {
        vrna_md_t md;
...
params = vrna_params(&md);
    ...
    for (const auto &[s, seq] : strands) {
        vrna_fold_compound_t *fc = vrna_fold_compound(seq.c_str(), nullptr, VRNA_OPTION_MFE);
        fold_compounds[s] = fc;
        ...
        for (int i = 1; i <= len; ++i) {
            for (int j = i; j <= len; ++j) {
                int idx = fc->iindx[i] - j; // ViennaRNA 1D index
...
int RNAEnergyEvaluator::hairpin_energy(int i, int j, int s) const {
    ...
    return vrna_E_hairpin(u, type, S1_map.at(s)[i + 1], S1_map.at(s)[j - 1], strands.at(s).c_str() + i - 1, params);
}
