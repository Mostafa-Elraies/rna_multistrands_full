...
// Building dot-bracket per strand and join by '&'.
static std::string dotbracket_from_output(
  ...
  for (const auto& p : bt.list_of_pairs) {
    ...
    if (x == y) {
      ...
      // Cross-strand: puting '(' on earlier strand in realized order
...
// Running Vienna cofold MFE on "A&B" and return (structure, mfe)
...
int main() {
  try {
    ...
    std::vector<Test> tests = {
      ...
      {"AAAAAA", "AAAAAA"},     // should NOT pair 
      {"CCCCCC", "CCCCCC"},     // should NOT pair
      {"AAAAAA", "CCCCCC"},     // should NOT pair 
      {"GCAAAA", "UUUGCU"},     // mixed
    };
